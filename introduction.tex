...
Along with the syntactic derivation, \ccg builds up a lambda expression denoting the semantic structure of the sentence (not shown in this figure). %TODO: Add lambda expressions to figure
...
Tools for reversing these conversions enable performance comparisons between parsers based on the two formalisms, and provide more options for researchers and developers building pipelines in which parsing is only one step.
However, reversing conversions is difficult, as the original conversion may have lost information or smoothed over inconsistencies in the corpus.
\textcite{Clark-Curran:2009} developed a \ccg to \ptb conversion that treats the \ccg derivation as a phrase structure tree and applies hand-crafted rules to every pair of categories that combine in the derivation.
...
Even with such rules, they correctly convert only 39.7\% of gold CCGbank derivations.
...
Our approach perfectly converts 51.4\% of sentences, an 11.7\% (absolute) improvement over \textcite{Clark-Curran:2009}.
On the remaining sentences our conversion generally handles most of the sentence correctly, but makes mistakes on some clause spans and rare spans such as QPs, NXs, and NACs.
Many of these errors are inconsistencies in the original \ptb annotations that are not recoverable.
These issues make evaluating parser output difficult, but using our method we do an improved comparison of \ccg and \ptb parsers.
