...
Our representation is based on \cite{cck}'s tree representation, with three key differences:
...
\cite{cck} use r-adjunction to add additional non-terminals to spines.
...
Starting from \cite{cck}'s head rules, we made modifications to avoid most cycles.
...
We generalize \cite{ec}'s definition of one-endpoint crossing trees to graphs.
...
%%%    + Gapping shift & 1.81 & 0.38 \\
  ...
  \caption{\label{tab:coverage}
    ...
    Core uses the representation proposed in this paper, with the head rules from \cite{cck}.
...
Each row shows the outcome of an addition to the previous row, starting from no traces at all, going to our representation with the head rules of \cite{cck}, then changing the head rules, reversing null-null edges, and changing the target of edges in gapping.
The largest gain comes from changing the head rules, which is unsurprising since \cite{cck}'s rules were designed for trees, where any set of rules produce valid structures.
