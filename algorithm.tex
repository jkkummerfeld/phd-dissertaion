...
In this section we sketch our algorithm, focusing on how it differs from \cite{ec}'s tree parsing algorithm.
...
Rather than the conventional constituency parsing approach where items fully cover words and end in the gaps between words, our items fully cover the gaps and end on the words, as shown in Figure~\ref{fig:alg-example}, and like in \cite{eisner-satta-1999}'s algorithm.
...
Comparing these definitions with the items in \cite{ec}, we have one entirely new type, the Exterval, and we have added the requirement that the later items have at least one crossing of the specified type.
...
Since our item types are entirely disjoint, throughout the rules below we need to give multiple options for item types in many places where \cite{ec} give only one type.
...
If $|C| = 1$, let $t$ be the endpoint outside $ps$ (this is one source of ambiguity in \cite{ec}).
...
This structure does not come up in \cite{ec}'s algorithm because of tree constraints that apply once directionality is considered.
...
The choice of split point in this case is one source of derivational ambiguity in \cite{ec}.
...
Our decomposition of this item differs from \cite{ec}'s in several ways because of our decision to separate edge creation and item combination.
...
As in the previous section, our decomposition differs from \cite{ec}'s to meet our altered definition of $N$.
...
\cite{cck}'s projective tree parser uses a similar approach, while \cite{ec}'s algorithm is for dependency parsing and so does not consider spines.
...
One key difference between our algorithm and the original one-endpoint-crossing algorithm presented by \cite{ec} is that we avoid spurious ambiguity in the ways a parse can be decomposed.
...
\section{Complete Dynamic Program}
...
\section{Full Algorithm Derivation}
