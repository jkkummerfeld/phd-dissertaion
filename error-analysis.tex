...
\State
\State $G = \emptyset$ \Comment{Transformation stage (Section~\ref{sec:tree-transform})}
...
\EndFor
\State
\ForAll{groups $g \in G$} \Comment{Classification stage (Section~\ref{sec:classify})}
...
\subsection{Transformations} \label{sec:tree-transform}
...
For example, what appears as a missing unary production may correspond to an extra bracket that contains a node that attached incorrectly.
...
A generative lexicalized parser, with three models, a base model, a model that uses subcategorization frames for head words, and a model that takes into account traces.
...
\centering
\input{figures/error-chinese-NP-internal}
\end{minipage}\hfill
\begin{minipage}[b]{0.5\textwidth}
\centering
\input{figures/error-chinese-modifier-attach}
\end{minipage}
\end{figure}
...
\begin{minipage}[b]{0.45\textwidth}
\centering
\input{figures/error-chinese-sense-confusion}
\end{minipage}\hfill
\begin{minipage}[b]{0.5\textwidth}
\centering
\input{figures/error-chinese-verb-args}
\end{minipage}
\end{figure}
...
(IP (ADVP \mbox{\glos{仅}{Only}}) (NP \mbox{\glos{去年}{last year,}}) (NP \mbox{\glos{中国 银行}{Bank of China}}) \ldots), \\
where the ADVP should modify the first NP only, which can be indicated by adding an NP to form (NP (ADVP \mbox{\glos{仅}{Only}}) (NP \mbox{\glos{去年}{last year}})).
...
\paragraph{Bikel} \parencite{Bikel-Chiang:2000:CLP}.
