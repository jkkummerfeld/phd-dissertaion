...
\begin{tabular}{|lccccr|}
	\hline
	  % TODO: Move F to be after P and R
...
\end{tabular}
\caption{
	\label{tab:standard-results} \parseval results on \wsj section 23 for the parsers we consider.
}{ 
	\parseval results on \wsj section 23 for the parsers we consider.
	% TODO: Change (if changed above)
...
\derivspace
\caption{
	\label{fig:PP-attachment} Error analysis example: PP Attachment.
}{
...
% TODO: Change \emph throughout to instead use my font for text examples (and in figures too)
...
We began with a large set of node errors, in the first stage they were placed into groups, one group per tree transformation used to get from the test tree to the gold tree.
...
\derivspace
\caption{
	\label{fig:NP-attachment} Error analysis example: NP Attachment.
}{
...
\derivspace
\caption{
	\label{fig:modifier-attachment} Error analysis example: modifier attachment.
}{
...
[VP
	...
	[\wrongnode{S}
		[\wrongnode{VP}
			...
			[\wrongnode{VP}
				[VB [restrict]]
				[NP [.t the RTC to \ldots]] % TODO: Avoid ldots, get text
...
\derivspace
\caption{
	\label{fig:clause-attachment} Error analysis example: clause attachment.
}{
...
\derivspace
\caption{
	\label{fig:unary} Error analysis example: unary.
}{
...
\derivspace
\caption{
	\label{fig:coordination} Error analysis example: coordination.
}{
...
\derivspace
\caption{
	\label{fig:NP-internal-structure}  Error analysis example: NP internal structure.
}{
...
\caption{
	...
	Variation in size and contents of the domains we consider.
}{
...
\end{tabular}
\caption{
  \label{tab:errors} Breakdown of errors in Chinese parsing.
}{
...
\centering
  ...
  \caption{
    \label{fig:np_internal} Error analysis example: NP internal (Chinese).
  }{
...
\centering
  ...
  \caption{
    \label{fig:wrong_arg} Error analysis example: Verb taking wrong arguments (Chinese).
  }{
...
\centering
  ...
  \caption{
    \label{fig:mod_att} Error analysis example: modifier attachment (Chinese).
  }{
...
\centering
  ...
  \caption{
    \label{fig:sense} Error analysis example: Word sense confusion (Chinese).
  }{
...
\end{tabular}
\caption{
  \label{tab:comparison} Error breakdown for a range of parsers on the \pctb.
}{
...
\begin{table}
  ...
  \caption{\label{tab:pos-confusion}
    The most frequently confused POS tag pairs in Chinese parsing.
  }{
